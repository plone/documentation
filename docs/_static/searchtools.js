...
const _finishSearch = (resultCount) => {
  ...
  // );
  else Search.status.innerText = `${resultCount} page(s) found.`;
};
...
const Search = {
  ...
  init: () => {
    ...
    const doc_section = new URLSearchParams(window.location.search).get(
      "doc_section"
    );
    document
    ...
    document
      .querySelector('input[id="doc_section_' + doc_section + '"]')
      .checked = true;
    if (query) Search.performSearch(query, doc_section);
  },
   ...
   */
  performSearch: (query, doc_section) => {
    ...
    if (Search.hasIndex()) Search.query(query, doc_section);
