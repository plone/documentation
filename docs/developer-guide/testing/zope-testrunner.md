...
```shell
zope-testrunner --test-path src -s my.addon
```

`-s` takes the dotted name of the package to search for tests.
`--test-path` points the runner at the directory that holds your source, usually `src`.
This is the form the `tox` environments generated by [plone.meta](https://github.com/plone/meta) use.

If your package is instead pip-installed in development mode, `--auto-path` locates its directory for you, so you do not have to name a path:

```shell
zope-testrunner --auto-path -s my.addon
```

Add `--auto-color` and `--auto-progress` to either form for readable output:

```shell
zope-testrunner --auto-color --auto-progress --test-path src -s my.addon
```
