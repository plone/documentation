...
📌 **Before running any `start` command, execute `make install` to avoid missing dependencies.**  

🐳 **If using Docker, prefer `make stack-start` instead of manually starting frontend and backend.**  

---

📌 **Tip:** If you're unsure about a command, run `make help` to see all available options.  
